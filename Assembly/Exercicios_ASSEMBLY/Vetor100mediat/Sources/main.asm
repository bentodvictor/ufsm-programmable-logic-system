...
inicio:
          ...
          lda PTAD     ;Acc <= PTA
          sta lido,x   ;lido[x] <= Acc
          dbnzx inicio ; x--, se x!=0 pula pra inicio
          lda PTAD     ;Acc <= PTAD
          sta lido,X   ;lido[0] <= Acc
   
media:   
          ldx #99
  
loopmedia:
          lda sum+1 ;Acc<=soma[l]
          add lido,x ;Acc<= soma[l] + lido[x]
          sta sum+1   ;sum[1] <= acc
          
          lda sum
          ADC #0     ;acc <= soma[h]  + 0 + carry 
          sta sum
          dbnzx loopmedia  ;x--, se x != 0 pula pra media
...
divide: 
          ldhx sum   ;carrega h<=sum[h] e x<=sum[l]
          txa       ; A<=x  ou usar lda sum+1
          ldx #100

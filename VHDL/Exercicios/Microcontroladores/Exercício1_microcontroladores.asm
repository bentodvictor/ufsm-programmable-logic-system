...
main     
           ...
           MOV    #$FF, PTBDD           ; PTB й saida
           SEI                          ; desliga interr.
           
loop:      
		 LDA PTAD; 	Acumulador recebe porta A
		 STA A;		Variavel A recebe acumulador
		 ADD A;		incrementa acumulador AC <- 2a
		 STA temp;	temp <- 2a
		 ADD temp;	incrementa acumulador AC <- 4a
		 STA temp;	temp <- 4a
		 ADD temp;	incrementa acumulador AC <- 8a
		 ADD A;		AC <- 9a
